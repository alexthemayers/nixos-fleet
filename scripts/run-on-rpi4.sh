...
trap 'rm -f "$remote_script"' EXIT
{
  ...
  for v in ATTIC_SKIP_IF_CACHED ATTIC_TOOLING_ONLY ATTIC_SKIP_FILL ATTIC_FORCE_SWITCH ATTIC_PUSH_JOBS ATTIC_PUSH_BATCH_SIZE ATTIC_CACHE_URL ATTIC_ENDPOINT ATTIC_COPY_FROM_BUILDER; do
    if [ -n "${!v:-}" ]; then
      printf 'export %s=%q\n' "$v" "${!v}"
    fi
  done

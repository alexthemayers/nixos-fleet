{ config, pkgs, ... }:
let
  alexKey = "ssh-rsa  a.mayers102@gmail.com";
  gitlabCiKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBWtDD746/XNq+8pWKb5I2MMEi6QarWmu0UNl7be7akX gitlab-ci-deploy";
in

{
  services = {
    openssh = {
      enable = true;
      openFirewall = true;
      startWhenNeeded = true;
      ports = [ 22 ];
      settings = {
        PasswordAuthentication = false;
        PermitRootLogin = "prohibit-password";
        X11Forwarding = false;
      };
    };
  };
  users = {
    users.alex = {
      openssh.authorizedKeys.keys = [ alexKey ];
      isNormalUser = true;
      description = "alex";
      extraGroups = [
        "wheel"
        "docker"
        "render" # TODO: move to jellyfin
      ];
      shell = pkgs.zsh;
      packages = with pkgs; [ ];
    };
    users.root = {
      openssh.authorizedKeys.keys = [
        alexKey
        gitlabCiKey
      ];
      shell = pkgs.zsh;
    };
  };
}
